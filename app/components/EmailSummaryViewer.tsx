...
function ProductSelect({ value, onChange, id = "email-product-select" }: ProductSelectProps) {
  return (
...
function InsightGrid({ data }: { data: SummaryResponse }) {
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <section className="glass-card rounded-2xl p-6 border border-emerald-400/30 neon-glow-blue">
        <h3 className="text-lg font-bold text-emerald-300 mb-4 flex items-center gap-2">
          <span className="text-2xl">💬</span> Positive signals
        </h3>
      ...
      <section className="glass-card rounded-2xl p-6 border border-red-400/30 neon-glow-magenta">
        <h3 className="text-lg font-bold text-red-300 mb-4 flex items-center gap-2">
          <span className="text-2xl">⚠️</span> Pain points
        </h3>
        <ul className="space-y-2">
...
export function EmailSummaryViewer({
  data,
  isLoading,
  onGenerate,
  selectedProduct,
  onProductChange,
  generatedForProduct,
}: EmailSummaryViewerProps) {
  if (!data && !isLoading) {
    return (
      <div className="flex flex-col items-center justify-center text-center text-cyan-100/60 min-h-[320px] space-y-5">
        <p className="text-xl font-semibold text-cyan-200">Generate inbox insights</p>
        <p className="max-w-md text-cyan-100/70">
          Pull an executive summary from recent customer emails to understand sentiment, emerging themes, and what to tackle next.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-end gap-3 w-full max-w-xl pt-2">
          ...
          >
            {selectedProduct === "all" ? "Analyze All Emails" : "Generate Summary"}
          </button>
        </div>
        <p className="text-xs text-cyan-100/60">Tip: keep it broad for overall inbox health or pick a model to zoom in.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-end justify-between gap-4 pb-4 border-b border-gray-200/10">
        <ProductSelect value={selectedProduct} onChange={onProductChange} id="email-product-select-loaded" />
        <button
          onClick={onGenerate}
        ...
        >
          {selectedProduct === generatedForProduct ? "Refresh Insights" : "Generate Summary"}
        </button>
      </div>
      
      <section className="glass-card rounded-2xl p-6 border border-cyan-400/30 neon-glow-cyan">
        <h3 className="text-lg font-bold text-cyan-300 mb-3 flex items-center gap-2">
          <span className="text-2xl">📧</span> Executive summary from inbox
        </h3>
